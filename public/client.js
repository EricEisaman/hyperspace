...
window.environment.setAttribute('environment',`preset:${window.config.environment.preset}`);
Object.keys(window.config.environment).forEach(key=>{
  if(!window.config.environment[key].length || key=="preset")return;
...
document.querySelector('a-scene').addEventListener('loaded', function () {
  ...
  sky.setAttribute('radius',218);
  let ground = window.environment.components.environment.ground;
  //ground.setAttribute('scale','2.1 2.1 2.1');
  ground.setAttribute('geometry','depth:0.1 ;height:400 ;width:400');
