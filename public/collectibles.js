  ...
  collectibleDefs.forEach((def,index)=>{
    ...
    for(let i=0; i<def.positions.length; i++){
      ...
      if(typeof def.animation != 'undefined'){
        switch(def.animation.type){
          case 'custom': col.setAttribute('animation-mixer',`clip:${def.animation.clip}`)
            break;
          case 'upDown':
            let m = document.createElement('a-animation');
            let p = def.positions[i].split(' ');
            let p0 = `${p[0]} ${p[1]} ${p[2]}`;
            let p1 = `${p[0]} ${p[1] + 1} ${p[2]}`;
            m.setAttribute('attribute','position');
            m.setAttribute('from', p0);
            m.setAttribute('to',p1);
            m.setAttribute('direction','alternate');
            m.setAttribute('dur','1000');
            m.setAttribute('repeat','indefinite');
            m.setAttribute('easing','linear');
            col.appendChild(m); 
            break;
          default:
